        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    @foreach($cursos_personas as $cp)
                      <form method="POST" action="{{ route('cursos_personas.modificar')}}">
                        @csrf
                        <div class="grid grid-cols-3 gap-4">
                        	<div class="form-group mb-6">
                            	<label for="exampleInputEmail1" class="form-label inline-block mb-2 text-gray-700">Guardia</label>
                                <br>
                                <label for="exampleInputEmail1" class="form-label inline-block mb-2 text-gray-700 font-bold">{{ $cp->nombres }}</label>
                                <input type="hidden" name="pers_nrut" id="pers_nrut" value="{{ $cp->pers_nrut }}">
                                <input type="hidden" name="cupe_ncod" id="cupe_ncod" value="{{ $cp->cupe_ncod }}">
                            </div>
                            <div class="form-group mb-6">
	                            ...
	                            <select class="form-select form-select-lg mb-3
                                    ...
                                    @foreach($cursos as $c)
                                        <option value="{{ $c->curs_ncod}}"
                                            @if( old('curs_ncod') == $c->curs_ncod || $cp->curs_ncod == $c->curs_ncod)
                                                selected
                                            @endif
                                ...
                                <div class="flex items-center justify-center">
                                  <div class="datepicker relative form-floating mb-3 xl:w-96" data-mdb-toggle-button="false">
                                    <input type="text"
                                      ...
                                      placeholder="" data-mdb-toggle="datepicker" name="cupe_fexpira" id="cupe_fexpira" value="{{ old('cupe_fexpira',$cp->cupe_fexpira) }}" />
